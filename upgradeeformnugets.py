...
def main():
    ...
    if int(get_git_status()) > 0:
        ...
        os.chdir("eFormAPI/Plugins/Monitoring.Pn/Monitoring.Pn")

        current_number_of_commits = get_commit_count()
        packages = [
            'Microting.eForm', 'Microting.eFormApi.BasePn', 'Microting.EformMonitoringBase'
        ]
        process_repository('Monitoring.Pn.csproj', packages, 'eform-angular-monitoring-plugin')
        ...
        if new_number_of_commits > current_number_of_commits:
            ...
            os.chdir("../../../..")
            run_command(f"github_changelog_generator -u microting -p eform-angular-monitoring-plugin -t {os.getenv('CHANGELOG_GITHUB_TOKEN')}", shell=True)
