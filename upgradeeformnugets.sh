...
if (( "$GIT_STATUS" > 0 )); then
	git pull
	cd eFormAPI/Plugins/Monitoring.Pn/Monitoring.Pn

	dotnet add Monitoring.Pn.csproj package Microting.eForm
	dotnet add Monitoring.Pn.csproj package Microting.eFormApi.BasePn
	dotnet add Monitoring.Pn.csproj package Microting.EformMonitoringBase
	...
	EFORM_PLUGINBASE_VERSION=`dotnet list package | grep 'Microting.EformMonitoringBase' | cut -c64-71`
	COMMIT_MESSAGE="Updating"$'\n'"- Microting.eForm to ${EFORM_VERSION}"$'\n'"- Microting.eFormApi.BasePn to ${EFORM_BASEPN_VERSION}"$'\n'"- Microting.EformMonitoringBase to ${EFORM_PLUGINBASE_VERSION}"
